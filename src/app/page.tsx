...
const ZONE_THEMES: Record<string, { bg: string; glow: string; text: string }> = {
...
export default function Home() {
  ...
  const { totalItems, setIsOpen, isOpen, totalAmount } = useCart();
  ...
  const [settings, setSettings] = useState<Record<string, string>>({
    ...
    hero_badge_text: lang === 'ru' ? 'Ваш гид по витаминам' : 'Роҳнамои шумо дар олами витаминҳо',
    ...
    async function loadSettings() {
      try {
        ...
        if (data && data.length > 0) {
          ...
          data.forEach((s: any) => sMap[s.key] = s.value);
  ...
  return (
    <main className="flex-1 flex flex-col relative bg-[#FDFBF7] text-[#1D1D1F] selection:bg-[#1E40AF] selection:text-white font-sans antialiased">
          ...
          <AnimatePresence mode="wait">
            {isSearchOpen ? (
              ...
              >
                 ...
                 >
                    <div className="w-12 h-12 md:w-13 md:h-13 rounded-2xl bg-white shadow-sm border border-black/[0.03] flex items-center justify-center p-0 transition-all group-hover:scale-110 group-active:scale-95 duration-500 overflow-hidden">
                      <img src="/logo.webp" alt={`${settings.brand_name} Logo`} className="w-full h-full object-contain scale-[3.0]" />
                    </div>
                    ...
                    <AnimatePresence>
                      {isMounted && totalItems() > 0 && (
                        <motion.span
                          ...
                          animate={{ scale: 1 }}
                          exit={{ scale: 0 }}
                          className="absolute -top-1 -right-1 w-5 h-5 bg-[#1D1D1F] text-white text-[9px] font-bold rounded-full flex items-center justify-center border-2 border-white shadow-xl"
                        >
                          {totalItems()}
                        </motion.span>
                      )}
        ...
        <AnimatePresence mode="wait">
          {!isSearchActive && (
            <motion.div 
              ...
              className="overflow-hidden"
            >
              <div id="quiz" className={`${isImmersiveMode ? 'min-h-[85vh] flex items-center pt-0' : 'pb-24'}`}>
                <QuizEngine 
                  whatsappNumber={settings.whatsapp_phone} 
                  lang={lang} 
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-16 mb-16">
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-3">
                  <button onClick={() => document.getElementById('quiz')?.scrollIntoView({ behavior: 'smooth' })} className="block text-[14px] hover:text-white transition-colors">
                    {lang === 'ru' ? '🧬 Персональный подбор' : '🧬 Интихоби инфиродӣ'}
                  </button>
                  <button onClick={() => document.getElementById('catalog')?.scrollIntoView({ behavior: 'smooth' })} className="block text-[14px] hover:text-white transition-colors">
