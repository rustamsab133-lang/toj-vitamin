...
* Товары отсутствующие в локальном JSON-словаре (из 107 товаров ${notInEnrichedData.length} не имеют метаданных из PDF): **${notInEnrichedData.length}**

## 🖼️ Нет фотографий (${missingImages.length})
${missingImages.length === 0 ? 'Все фото на месте! ✅' : missingImages.map(n => '- ' + n).join('\n')}

## 📝 Нет описания (Properties / Marketing Hooks) (${missingDescriptions.length})
${missingDescriptions.length === 0 ? 'У всех товаров есть описание! ✅' : missingDescriptions.map(n => '- ' + n).join('\n')}

<br>
<details>
<summary><b>Показать список ${notInEnrichedData.length} товаров без данных из PDF</b></summary>

${notInEnrichedData.map(n => '- ' + n).join('\n')}
</details>
  `;
